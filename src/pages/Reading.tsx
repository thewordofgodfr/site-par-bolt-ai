...
export default function Reading() {
  ...
  // Hauteur de la nav : un peu plus haute sur mobile (barre mobile + sous-nav)
  const NAV_H = 64;

  // Mesure de la barre de recherche (sticky) pour calculer l'offset total
  const searchRef = useRef<HTMLDivElement>(null);
  const [searchH, setSearchH] = useState(0);

  // Mesure du bandeau commandes (sticky) pour calculer l'offset total
  ...
  // Mesurer la hauteur des barres sticky (recherche + commandes)
  useEffect(() => {
    const compute = () => {
      setSearchH(searchRef.current?.offsetHeight || 0);
      setCmdH(commandBarRef.current?.offsetHeight || 0);
    };
    compute();
  ...
  // ---- Offset total pour le scroll/centrage des versets
  const stickyOffset = NAV_H + searchH + cmdH + 12; // +12px de marge visuelle

  // Scroll + centrage fiable (évite d'être masqué par les barres collantes)
  useEffect(() => {
    if (highlightedVerse !== null && chapter) {
      ...
      const timer = setTimeout(() => {
        ...
        const target = current + rect.top - (NAV_H + searchH + cmdH) - 8; // petit tampon
        window.scrollTo({ top: Math.max(target, 0), behavior: 'smooth' });
      }, 120);
      return () => clearTimeout(timer);
    }
  }, [chapter, highlightedVerse, NAV_H, searchH, cmdH]);

  // Extinction de la surbrillance après 20s
  useEffect(() => {
                    ...
                    {chapter.verses.map((v, idx) => {
                      ...
                      return (
                        <div
                          ...
                          // marge de scroll pour que le verset n'arrive pas sous les barres sticky
