...
export default function Reading() {
  ...
  // Mise en évidence (ex. depuis une recherche ou verset aléatoire)
  ...
  useEffect(() => {
    if (!hasLoadedContext && state.readingContext && state.readingContext.book && state.readingContext.chapter > 0) {
      ...
      if (book) {
        ...
        setShowRestoredNotification(false);
        setSelectedVerses([]);
        // Surbrillance si un verset est fourni
  ...
  // Scroll + surbrillance (20s) quand on vient d'une recherche / aléatoire
  useEffect(() => {
  ...
  return (
    <div className={`min-h-screen ${isDark ? 'bg-gray-900' : 'bg-gray-50'} transition-colors duration-200`}>
      {/* Bandeau commandes (Livres + chapitre) — sticky juste sous la nav */}
      {selectedBook && (
        <div className="sticky top-20 sm:top-16 z-30 bg-transparent">
      ...
      <div className="container mx-auto px-4 pb-6">
        <div className="max-w-6xl mx-auto">
          ...
          {selectedVerses.length > 0 && (
            <div className={`hidden sm:flex mt-3 mb-4 rounded-lg p-3 items-center justify-between ${isDark ? 'bg-gray-800 text-gray-100' : 'bg-white text-gray-800'} shadow`}>
          ...
          {/* Contenu du chapitre */}
          {selectedBook && (
            <div className={`${isDark ? 'bg-gray-800' : 'bg-white'} rounded-xl shadow-lg p-6 min-h-96 mt-3`}>
